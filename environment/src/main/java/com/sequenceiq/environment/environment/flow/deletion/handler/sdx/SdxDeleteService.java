...
public class SdxDeleteService {
    ...
    public SdxDeleteService(CrnAwareSdxConnector crnAwareSdxConnector,
    ...
    private void waitSdxClustersDeletion(PollingConfig pollingConfig, EnvironmentView environment, Set<String> sdxCrns, boolean force) {
        ...
        sdxCrns.forEach(crn -> crnAwareSdxConnector.delete(crn, force));

        LOGGER.debug("Starting poller to check all Data Lake stacks for environment {} is deleted", environment.getName());
        pollingDeletion(pollingConfig, crnAwareSdxConnector.getAttemptResultForDeletion(sdxCrns, environment.getName(), environment.getResourceCrn()));
    }
    ...
    private void pollingDeletion(PollingConfig pollingConfig, AttemptResult pollingAttempt) {
        try {
