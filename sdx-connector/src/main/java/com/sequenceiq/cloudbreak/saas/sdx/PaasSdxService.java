...
public class PaasSdxService extends AbstractSdxService<SdxClusterStatusResponse> {
    ...
    public Set<String> listSdxCrns(String environmentName, String environmentCrn) {
        return getSdxClusterResponseStream(environmentName, environmentCrn)
                .map(SdxClusterResponse::getCrn)
                .collect(Collectors.toSet());
    }

    @Override
    public Set<Pair<String, SdxClusterStatusResponse>> listSdxCrnStatusPair(String environmentName, String environmentCrn) {
        return getSdxClusterResponseStream(environmentName, environmentCrn)
    ...
    @Override
    public PollingResult getDeletePollingResultByInstanceStatus(SdxClusterStatusResponse status) {
        return DELETE_FAILED.equals(status) ? PollingResult.FAILED : PollingResult.IN_PROGRESS;
    }

    private Stream<SdxClusterResponse> getSdxClusterResponseStream(String environmentName, String environmentCrn) {
