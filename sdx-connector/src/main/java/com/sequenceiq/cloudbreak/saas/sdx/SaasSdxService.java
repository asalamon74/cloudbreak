...
public class SaasSdxService extends AbstractSdxService<SDXSvcAdminProto.InstanceHighLevelStatus.Value> {
    ...
    @Override
    public CrnResourceDescriptor crnResourceDescriptor() {
        return CrnResourceDescriptor.SDX_SAAS_INSTANCE;
    }
    ...
    public Set<String> listSdxCrns(String environmentName, String environmentCrn) {
        if (isPlatformEntitled(Crn.safeFromString(environmentCrn).getAccountId())) {
            return getInstanceStream(environmentCrn)
    ...
    @Override
    public Set<Pair<String, SDXSvcAdminProto.InstanceHighLevelStatus.Value>> listSdxCrnStatusPair(String environmentName, String environmentCrn) {
        if (isPlatformEntitled(Crn.safeFromString(environmentCrn).getAccountId())) {
            return getInstanceStream(environmentCrn)
    ...
    @Override
    public PollingResult getDeletePollingResultByInstanceStatus(SDXSvcAdminProto.InstanceHighLevelStatus.Value status) {
    ...
    private Stream<SDXSvcAdminProto.Instance> getInstanceStream(String environmentCrn) {
        if (isPlatformEntitled(Crn.safeFromString(environmentCrn).getAccountId())) {
